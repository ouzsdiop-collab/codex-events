...
// Seuils "sains" (repères 6-max, pédagogiques).
const BANDS = {
  pfAcc:  { good: 0.75, warn: 0.60 },        // précision préflop
  vpip:   { lo: 20, hi: 30, hardLo: 15, hardHi: 36 },
  pfr:    { lo: 15, hi: 24, hardLo: 10, hardHi: 30 },
  gap:    { warn: 8, bad: 14 },              // VPIP - PFR (passivité préflop)
...
export function computeProfile(state) {
  ...
  if (hands >= 25 && vpip != null && af != null) {
    if (vpip < 22 && af >= 1.5) style = 'Serré-agressif (TAG) — bonne base solide';
    else if (vpip > 30 && af >= 1.5) style = 'Large-agressif (LAG) — rentable mais volatil';
    else if (vpip < 22 && af < 1.3) style = 'Serré-passif (nit) — tu rates de la value';
    else if (vpip > 28 && af < 1.2) style = 'Large-passif (calling station) — la fuite la plus chère';
  ...
  const metrics = [
    metric('Précision préflop', pfAcc, v => (v * 100).toFixed(0) + '%',
      ...
      'Vise > 75 %', spots, 10),
    metric('VPIP', vpip, v => v.toFixed(0) + '%', pctStatusBand(vpip, BANDS.vpip), 'Idéal 20–30 %', hands, 15),
    metric('PFR', pfr, v => v.toFixed(0) + '%', pctStatusBand(pfr, BANDS.pfr), 'Idéal 15–24 %', hands, 15),
    ...
    metric('Abattage (WTSD)', wtsd, v => v.toFixed(0) + '%', pctStatusBand(wtsd, BANDS.wtsd), 'Idéal 22–32 %', hands, 20),
    metric('Agressivité postflop (AF)', af, v => (v >= 99 ? '∞' : v.toFixed(1)),
      ...
      'Idéal 1.3–3.5', t.postflopBets + t.postflopCalls, 12),
  ];
...
export function generateStrategy(profile) {
  ...
  if (sample.hands >= 25) {
    ...
    if (vpip != null && vpip > BANDS.vpip.hardHi) chantiers.push(mChant(3.2, `VPIP trop élevé (${vpip.toFixed(0)} %)`, `Tu entres dans trop de pots.`, 'Resserre vers 22–27 % : élimine les mains offsuit faibles et les tirages spéculatifs hors position.'));
    else if (vpip != null && vpip < BANDS.vpip.hardLo) chantiers.push(mChant(2.4, `VPIP trop bas (${vpip.toFixed(0)} %)`, `Tu joues trop peu de mains.`, 'Élargis en late position (CO/BTN) : tu laisses des blindes non contestées.'));
